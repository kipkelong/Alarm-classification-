...
Db<-function(db)
{
  ...
  if (db==1)
  {res<-dbSendQuery(con,'SELECT * FROM ActiveAlarms WHERE alarm_number BETWEEN 7600 AND 7610 LIMIT 100000')}
  if (db==2)
  res<-dbSendQuery(con,'SELECT * FROM CeasedAlarms WHERE alarm_number BETWEEN 7600 AND 7610 AND event_time >2019-07-01')
  else
...
{con <- dbConnect(RMySQL::MySQL(),
                   ...
                   host="172.29.229.190",
                   port=3306,
                   user="skibet",password="Monday_123")
    dbWriteTable(con, value = alarms_data, name = "TBL_ALARM", append = TRUE,overwrite=FALSE )
    #res<-dbSendQuery(con,'SELECT * FROM TBL_ALARM WHERE alarm_number BETWEEN 7600 AND 7610 LIMIT 100000')
  #disconnect db 
  dbDisconnect(con)}
Db_read<-function()
{con <- dbConnect(RMySQL::MySQL(),
...
ssh_connector<-function(hostip,issued_command)
{
  ...
  #host<-'skibet@10.153.7.79'
  host<-hostip
  session <- ssh_connect(host, keyfile = NULL, passwd = , verbose = FALSE)
  ssh_session_info(session)
  ssh_exec_wait(session, command = "su")
  ssh_exec_wait(session, command = "rastre1")
  #ssh_exec_wait(session, command = "zaho")
...
emailer<-function(destination_address,cc,alarm_text,Object_name,Instructions){
  from <- "skibet@safaricom.co.ke"
  to <- destination_address
  #to<-destination_address
  subject <-alarm_text
  body <- paste("Hi ", "\n", "kindly resolve  below alarm:","\t",Object_name,"\n","Resolution steps","\n",Instructions,"\n","Thanks in advance")                  
  mailControl=list(host.name ="172.29.229.99",port=587,user.name="skibet",passwd="!@$@#$",authenticate=TRUE,SSL=FALSE,send=TRUE)
...
Cost<-function(resolved_alarms_count)
{
  ...
  escort_savings<-(11/30+4/30)*resolved_alarms_count*12000
...
# Corr1$Status<-"Resolved"
# Corr1$Escalation_Date<-Sys.time()
# row=1
# while (row <nrow(Corr1)) {
# 
# Corr1$Site_ID[row]<-gsub("(.*)(.*)(.*)(.*)(.*)(.*)_.*","\\1",Corr1$symbolic_name[row])
# row<-row+1
# }
# 
# 
...
while (row <=nrow(df_replace_RF)){
  emailer("skibet@safaricom.co.ke","MNmureithi@safaricom.co.ke",df_replace_RF$additional_text[row],df_replace_RF$symbolic_name[row])
...
while (row <=nrow(df_replace_module)){
  emailer("skibet@safaricom.co.ke","MNmureithi@safaricom.co.ke",df_replace_module$additional_text[row],df_replace_module$symbolic_name[row])
  #df_replace_module$recurrency[row]<-count(df_replace_module$affected_NE[row])
  row<-row+1
  Sys.sleep(13)
}
...
while (row <=nrow(df_Site_visit)){
  emailer(BSC_info$`Support engineer`[BSC_info$`BSC NAME`==df_Site_visit$specific_problem[row]],BSC_info$`Support engineer`[BSC_info$`BSC NAME`==df_Site_visit$specific_problem[row]],df_Site_visit$additional_text[row],df_Site_visit$symbolic_name[row],df_Site_visit$Instruction[row])
  #df_replace_module$recurrency[row]<-count(df_replace_module$affected_NE[row])
  row<-row+1
  Sys.sleep(12)
}
#View(BSC_info$`Support engineer`[BSC_info$`BSC NAME`==df_Replace_alarm_box$specific_problem[5]])
#View(Spare_details[Spare_details$BCF=="52"&Spare_details$BSC=="395536",])
#View(Spare_details[Spare_details$BCF=="937"&Spare_details$BSC=="900649",])
